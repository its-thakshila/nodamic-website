...
export default memo(function OverlayUI({ isLoaded = false }) {
  ...
  return (
    <div
      className="absolute inset-0 w-full h-full px-6 py-6 sm:px-12 sm:py-10 md:px-16 md:py-12 flex flex-col lg:grid lg:grid-cols-12 lg:grid-rows-[auto_1fr_auto] pointer-events-none select-none"
    ...
    >
      ...
      {/* Flexible Spacer for Mobile — Pushes UI text below the 3D model */}
      <div className="flex-1 min-h-[45vh] sm:min-h-[50vh] lg:hidden" />

      {/* Row 3: Bottom layout — Stacked on mobile, Grid on desktop */}
      <div className="flex flex-col gap-6 sm:gap-8 lg:gap-0 lg:col-span-12 lg:row-start-3 lg:grid lg:grid-cols-12 lg:items-end lg:gap-x-[clamp(2rem,3.1vw,3rem)] pb-4 sm:pb-8 lg:pb-[clamp(3.3rem,5.2vw,5rem)] relative z-40 w-full">

        {/* Main hardware title */}
        <motion.div
          className="lg:col-span-7 self-start lg:self-end relative w-full -mt-16 sm:-mt-12 lg:-mt-0"
        ...
        <motion.div
          ...
          initial="hidden"
          animate={isLoaded ? "visible" : "hidden"}
          className="w-full lg:hidden -mt-2 sm:-mt-1 pointer-events-auto"
        >
          <button
